...
for j = 1:number_steps %outside time loop
    Time_array(j) = time_counter; %assign to time array
    time_counter = time_counter + time_step; %increment time counter
    for i = 1:number_particles %inside position loop
        ...
        [particle_state(i)] = Spatial_Switch_Decision(particle_state(i), particle_position(i), Axon_Length);
